...
## 自動検証
結果：Dev／Proto × Android 12L／17の各6件、計24件が成功。両APKのビルドとLintも成功（既存警告は残る）。静的解析の内部エラーは、新しいGradleプロセスで逐次実行して解消した。

通常の計装テストは各ビルド・各端末6件。ログイン画面、日本語リソース、Parcel、回答確認の値変換、回答率表示、全7種類の画面生成を確認。今回、リッカート・自由入力・単一選択・複数選択の入力欄が実際に表示されることも検証に追加。
...
## 残る確認
途中でMacが再ロックされたため、全設問・回答確認画面の最終目視、DevのGoogleログインからの実通信、Protoの既存アカウント接続・回答送信・Pushは未完了。文字拡大、横画面、Android 6（最低OS）の確認も必要。既存調査への送信・データ変更は行っていない。
